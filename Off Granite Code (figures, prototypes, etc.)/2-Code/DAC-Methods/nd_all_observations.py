...
def compute_observations(df: pd.DataFrame) -> Dict[str, float]:
    ...
    observations['percent_poc'] = (observations['num_poc'] / observations['num_patients']) * 100
    observations['percent_white'] = (observations['num_white'] / observations['num_patients']) * 100
    
    # Statistical tests results (correct)
    observations['num_hospitals_fep'] = sum(df['qmp'] < 0.05)
    observations['percent_hospitals_fep'] = (observations['num_hospitals_fep'] / observations['num_hospitals']) * 100
    observations['num_hospitals_mwp'] = sum(df['mwp'] < 0.05)
    observations['percent_hospitals_mwp'] = (observations['num_hospitals_mwp'] / observations['num_hospitals']) * 100
    ...
    for metric in ['pcd', 'pmpcd', 'wmpcd', 'dbdfe']:
        for threshold in [1, 5, 10]:
            if metric == 'pcd':
                ...
                count = sum((df['count_pmpcd_sig'] + df['count_wmpcd_sig']) >= threshold)
            else:
            ...
            observations[f'percent_hospitals_with_{threshold}_{metric}p_{suffix}'] = (count / observations['num_hospitals']) * 100
...
def validate_observations(observations: Dict) -> List[str]:
    ...
    required_variables = [
        'num_patients', 'num_hospitals',
        'num_poc', 'percent_poc', 'num_white', 'percent_white',
        'num_hospitals_fep', 'percent_hospitals_fep',
        'num_hospitals_mwp', 'percent_hospitals_mwp',
        'num_days_pmpcdp', 'percent_days_pmpcdp',
        'num_days_wmpcdp', 'percent_days_wmpcdp',
        'num_days_pcdp', 'percent_days_pcdp',
        'num_days_dbdfep', 'percent_days_dbdfep',
        'num_hospitals_with_1_pcdp_day', 'num_hospitals_with_10_pcdp_days',
        'percent_hospitals_with_1_pcdp_day', 'percent_hospitals_with_10_pcdp_days',
        'num_hospitals_with_1_pmpcdp_day', 'num_hospitals_with_10_pmpcdp_days',
        'percent_hospitals_with_1_pmpcdp_day', 'percent_hospitals_with_10_pmpcdp_days',
        'num_hospitals_with_1_wmpcdp_day', 'num_hospitals_with_10_wmpcdp_days',
        'percent_hospitals_with_1_wmpcdp_day', 'percent_hospitals_with_10_wmpcdp_days',
        'num_hospitals_with_1_dbdfep_day', 'num_hospitals_with_10_dbdfep_days',
        'percent_hospitals_with_1_dbdfep_day', 'percent_hospitals_with_10_dbdfep_days',
...
if __name__ == '__main__':
    summaries_path = './1-Input/Test-Data/summaries.csv'
    observations_path = './3-Output/Test-Data/observations.json'
