...
def compute_observations(df: pd.DataFrame, LOOKBACK_DAYS) -> Dict[str, float]:
    observations = {}
    ...
    total_days = observations["num_hospitals"] * LOOKBACK_DAYS 
    num_pmpcd_days = 0
    num_wmpcd_days = 0
    num_pcd_days = 0
    num_dbdfe_days = 0
...
def generate_observations(path_to_summaries, path_to_output, LOOKBACK_DAYS) -> Dict:
    df = pd.read_csv(path_to_summaries)
    observations = compute_observations(df, LOOKBACK_DAYS)
...
def main(PCD_THRESH, LOOKBACK_DAYS):
    ...
    observations_path = save_path / f"Export/observations_{PCD_THRESH}_LBD_{LOOKBACK_DAYS}.json"
    observations = generate_observations(summaries_path, observations_path, LOOKBACK_DAYS)
