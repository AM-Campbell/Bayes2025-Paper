...
def generate_example_hospital_results(hid, PCD_THRESH):
    results = {}
    example_hospital_id = hid
    function = 'HospiceTreat'
    udata = get_first_utilization_data(example_hospital_id, function, PCD_THRESH)
    ...
    print(f'{len(day_index_sup)} days are safe to export for hospital {hid}') 
    ...
    # Mann Whitney Results (FDH)
    assert n_w + n_p > 11
    results['mws'], results['mwp']= udata.get_mw()

    # NHD
    results['nhd_mws'], results['nhd_mwp']= udata.get_nhd_mw()
...
def main(PCD_THRESH):
    # hid = 330154 # Old HID from first export

    for hid in [390111, 450044]:
        print(f'getting example hospital results for {hid}')
        output_dir = Path('/drives/56219-Linux/56219dua/Project2017/4-Analysis/3-Analysis/AidanPaper/Export/')
        save_path = output_dir / f'example_hospital_results_{hid}_{PCD_THRESH}.json'
        results = generate_example_hospital_results(hid, PCD_THRESH)
        with open(save_path, 'w') as f:
            json.dump(results, f, cls=NumpyEncoder)
        print(f'Saved example hospital results to {save_path}')
