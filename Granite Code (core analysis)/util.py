...
class UtilizationData():
    ...
    def __init__(self, 
        ...
        w_days_count: List[int],
        p_days_count: List[int],
        PCD_THRESH: float):
        ...
        self.w_days_count = w_days_count
        self.p_days_count = p_days_count
        self.PCD_THRESH = PCD_THRESH
        ...
        self._wm_pcd: np.ndarray | None = None # probability difference > PCD_THRESH (w more)
        self._pm_pcd: np.ndarray | None = None # probability difference < -PCD_THRESH (poc more)
        ...
        self._nhd_mws: float | None = None # mann whitney u stat for num days comparison
        self._nhd_mwp: float | None = None # mann whitney p value for num days comparison
    ...
    def get_diff_dist(self):
        if self._diff_dist is None:
            ...
            w_post_samples = w_posteriors.rvs(size=(num_samples, 200), random_state=42)
            p_post_samples = p_posteriors.rvs(size=(num_samples, 200), random_state=42)
    ...
    def get_wm_pcd(self):
        if self._wm_pcd is None:
            self._wm_pcd = np.mean(self.get_diff_dist() > self.PCD_THRESH, axis=0)
        return self._wm_pcd
    ...
    def get_pm_pcd(self):
        if self._pm_pcd is None:
            self._pm_pcd = np.mean(self.get_diff_dist() < -self.PCD_THRESH, axis=0)
        return self._pm_pcd
    ...
    def get_nhd_mw(self):
        if (self._nhd_mws is not None) and (self._nhd_mwp is not None):
            return self._nhd_mws, self._nhd_mwp
        self._nhd_mws, self._nhd_mwp = mannwhitneyu(self.w_days_count, self.p_days_count)
        return self._nhd_mws, self._nhd_mwp
    ...
    def get_summary(self):
        ...
        mws, mwp = self.get_mw()
        nhd_mws, nhd_mwp = self.get_nhd_mw()
    ...
    def get_figure(self, supress=True):
        ...
        ax1.fill_between(t[mask], -1, 1, where=(self.get_wm_pcd()[mask] > 0.95), alpha=0.2, color=altcmap(norm(0.4)), label=f"P(Difference Signal > {self.PCD_THRESH}) > 0.95 (W More)")
        ax1.fill_between(t[mask], -1, 1, where=(self.get_pm_pcd()[mask] > 0.95), alpha=0.2, color=cmap(norm(0.8)), label=f"P(Difference Signal < -{self.PCD_THRESH}) > 0.95 (POC More)")
        ...
        plot_with_mask(ax2, t, self.get_wm_pcd(), mask, label=f"P(Difference Signal > {self.PCD_THRESH}) (W More)", color=altcmap(norm(0)))

        # ax2.plot(t[mask], self.get_pm_pcd()[mask], label="P(Difference Signal < -0.05) (POC More)", color=cmap(norm(1)))
        plot_with_mask(ax2, t, self.get_pm_pcd(), mask, label=f"P(Difference Signal < -{self.PCD_THRESH}) (POC More)", color=cmap(norm(1)))
...
def generate_n_file(function, lookback_days=200):
    ...
    def get_n_total_n_function(utilization_file_path: Path, function: str = function)   -> Tuple[int, int]:
        ...
        # applies lookback filter, does nothing if lookback_days == 200
        ut = apply_lookback_filter(ut, 200, lookback_days)
...
def apply_lookback_filter(df, default_lookback, lookback_days):
        assert lookback_days <= default_lookback
        if lookback_days < default_lookback:
            drop_before_day = default_lookback - lookback_days
            df = df[df['dayNumber'] >= drop_before_day]
        return df



def get_first_utilization_data(hospitalID:int , function: str, PCD_THRESH, lookback_days, pr = False):
    ...
    assert lookback_days <= 200, "lookback_days must be less than or equal to 200"

    if lookback_days < 200:
        # we need to modify the w_df and p_df to trim the data. 
        # if lookback period is 10 then we only want to show days 190 through 200
        print(f'Using lookback period {lookback_days}')
        w_df = apply_lookback_filter(w_df, 200, lookback_days)
        p_df = apply_lookback_filter(p_df, 200, lookback_days)
    ...
    # R&R get # of days in hospice for each pt who had hospice.
    w_days_count = fun_w_df.groupby('patientID')['dayNumber'].count().to_numpy()
    p_days_count = fun_p_df.groupby('patientID')['dayNumber'].count().to_numpy()

    # get number of pt who didn't get hospice
    n_w_no_hosp = n_w - len(w_first_days)
    n_p_no_hosp = n_p - len(p_first_days)

    # add 0 entries for patients who didn't get hospice
    w_days_count = np.append(w_days_count, np.zeros(n_w_no_hosp, dtype=int)).tolist()
    p_days_count = np.append(p_days_count, np.zeros(n_p_no_hosp, dtype=int)).tolist()

    assert len(w_days_count) == n_w
    assert len(p_days_count) == n_p
