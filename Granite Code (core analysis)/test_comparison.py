...
def compute_test_comparison(df: pd.DataFrame, LOOKBACK_DAYS) -> Dict:
    ...
    for _, row in df.iterrows():
        ...
        # All possible days
        # TODO: BUG, 200 is no longer always the corect range
        all_days = set(range(200-LOOKBACK_DAYS, 200))
    ...
    # R&R 1. NHD_MW v.s. 1 day PCD(+)
    nhd_mw_positive = df["nhd_mwp"] < 0.05
    for n in [1]:
        pcd_ge_n = (df["count_pmpcd_sig"] + df["count_wmpcd_sig"]) >= n
        pmpcd_ge_n = df["count_pmpcd_sig"] >= n
        wmpcd_ge_n = df["count_wmpcd_sig"] >= n


        comparisons[f"nhd_mwp_{n}_pcdp"] = int(sum(nhd_mw_positive & pcd_ge_n))
        comparisons[f"nhd_mwp_{n}_pcdn"] = int(sum(nhd_mw_positive & ~pcd_ge_n))
        comparisons[f"nhd_mwn_{n}_pcdp"] = int(sum(~nhd_mw_positive & pcd_ge_n))
        comparisons[f"nhd_mwn_{n}_pcdn"] = int(sum(~nhd_mw_positive & ~pcd_ge_n))


        comparisons[f"nhd_mwp_{n}_wmpcdp"] = int(sum(nhd_mw_positive & wmpcd_ge_n))
        comparisons[f"nhd_mwn_{n}_wmpcdp"] = int(sum(~nhd_mw_positive & wmpcd_ge_n))

        comparisons[f"nhd_mwp_{n}_pmpcdp"] = int(sum(nhd_mw_positive & pmpcd_ge_n))
        comparisons[f"nhd_mwn_{n}_pmpcdp"] = int(sum(~nhd_mw_positive & pmpcd_ge_n))

        comparisons[f'nhd_mwv{n}pcd_festat'], comparisons[f'nhd_mwv{n}pcd_fepval'] = stats.fisher_exact(
                np.array(
                    [
                        [comparisons[f"nhd_mwp_{n}_pcdp"], comparisons[f"nhd_mwp_{n}_pcdn"]],
                        [comparisons[f"nhd_mwn_{n}_pcdp"], comparisons[f"nhd_mwn_{n}_pcdn"]]
                    ]
                )
            )

    # R&R 2. NHD_MW v.s. FE (Simple contigency table)
    fe_positive = df["qmp"] < 0.05
    nhd_mw_positive = df["nhd_mwp"] < 0.05

    comparisons["fep_nhd_mwp"] = int(sum(fe_positive & nhd_mw_positive))
    comparisons["fep_nhd_mwn"] = int(sum(fe_positive & ~nhd_mw_positive))
    comparisons["fen_nhd_mwp"] = int(sum(~fe_positive & nhd_mw_positive))
    comparisons["fen_nhd_mwn"] = int(sum(~fe_positive & ~nhd_mw_positive))

    comparisons['fevnhd_mw_festat'], comparisons['fevnhd_mw_fepval'] = stats.fisher_exact(
            np.array(
                [
                    [comparisons["fep_nhd_mwp"], comparisons["fep_nhd_mwn"]],
                    [comparisons["fen_nhd_mwp"], comparisons["fen_nhd_mwn"]]
                 ]
                )
            )
...
def generate_test_comparison(path_to_summaries: str, path_to_output: str, LOOKBACK_DAYS):
    """Generate test comparison JSON file from summaries."""
    df = pd.read_csv(path_to_summaries)
    comparisons = compute_test_comparison(df, LOOKBACK_DAYS)
...
def main(PCD_THRESH, LOOKBACK_DAYS):
    save_path = Path(
        "/drives/drive1/56219dua/Project2017/4-Analysis/3-Analysis/AidanPaper"
    )
    summaries_path = save_path / f"summaries_{PCD_THRESH}_LBD_{LOOKBACK_DAYS}.csv"
    comparison_path = save_path / f"Export/test_comparison_{PCD_THRESH}_LBD_{LOOKBACK_DAYS}.json"

    generate_test_comparison(
        summaries_path, comparison_path, LOOKBACK_DAYS
    )
